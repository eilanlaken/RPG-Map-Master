...
public class Z_ModelMesh implements MemoryResource {
    ...
    public Z_ModelMesh(float[] positions, float[] uvs, @Deprecated float[] colors, float[] normals, float[] tangents, @Deprecated float[] biTangents, int[] indices, float boundingSphereRadius) {
    ...
    public static Z_ModelMesh createPlane(float width, float height, int subdivisionsHorizontal, int subdivisionsVertical) {
        ...
        return new Z_ModelMesh(positions.items, uvs.items, null, normals.items, tangents.items, biTangents.items, indices.items, radius);
    }

    /* CREATE A 3D CUBE */

    // hard-coded : totally fine.
    public static Z_ModelMesh createCube(float sizeX, float sizeY, float sizeZ) {
        float xh = sizeX / 2;
        float yh = sizeY / 2;
        float zh = sizeZ / 2;
        ...
        return new Z_ModelMesh(positions, uvs, null, normals, tangents, biTangents, indices, radius);
    }

    public static Z_ModelMesh createCubeInverted(float sizeX, float sizeY, float sizeZ) {
        float xh = sizeX / 2;
        float yh = sizeY / 2;
        float zh = sizeZ / 2;
