...
public class NodeInputCheckbox extends Node implements NodeInput<Boolean> {
    ...
    protected void renderBox(Renderer2D renderer2D, float x, float y, float deg, float sclX, float sclY) {
        float radius = MathUtils.clampFloat(cornerRadius, 0, BASE_SIZE * 0.5f);
        if (checked) renderer2D.setColor(checked ? borderColorChecked : borderColorUnchecked);
    ...
    protected void renderCheckmark(Renderer2D renderer2D, float x, float y, float deg, float sclX, float sclY) {
        float radius = MathUtils.clampFloat(cornerRadius, 0, BASE_SIZE * 0.5f);
